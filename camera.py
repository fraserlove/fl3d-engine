...
class Camera():
    def __init__(self, display):
        ...
        self.controls = {
            ...
            pygame.K_q: (lambda self, engine: self.engine.rotate(display.rotation_factor, 0, 0) if not self.text_boxes_accepting_input() else None),
            pygame.K_w: (lambda self, engine: self.engine.rotate(-display.rotation_factor, 0, 0) if not self.text_boxes_accepting_input() else None),
            pygame.K_a: (lambda self, engine: self.engine.rotate(0, display.rotation_factor, 0) if not self.text_boxes_accepting_input() else None),
            pygame.K_s: (lambda self, engine: self.engine.rotate(0, -display.rotation_factor, 0) if not self.text_boxes_accepting_input() else None),
            pygame.K_z: (lambda self, engine: self.engine.rotate(0, 0, display.rotation_factor) if not self.text_boxes_accepting_input() else None),
            pygame.K_x: (lambda self, engine: self.engine.rotate(0, 0, -display.rotation_factor) if not self.text_boxes_accepting_input() else None),
        }